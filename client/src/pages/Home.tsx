...
type ImportRow = { controlNo: string; studentId: string; firstName: string; lastName: string; middleName: string; email: string; program: string; yearLevel: number; barcode?: string };
...
export default function Home() {
  ...
  const { data: masterlists = [] } = trpc.imports.masterlists.useQuery();
  ...
  const importMutation = trpc.imports.students.useMutation({ onSuccess: async (result) => { toast.success("Import reviewed", { description: `${result.successfullyImported} students added to the roster. ${result.rejectedRows} rows rejected.` }); setImportPreview([]); setImportFilename(""); await Promise.all([utils.students.list.invalidate(), utils.imports.history.invalidate(), utils.imports.masterlists.invalidate()]); } });
  const resetRosterMutation = trpc.imports.resetRoster.useMutation({ onSuccess: async () => { toast.success("Current roster cleared", { description: "The masterlist history was preserved for reference." }); setShowResetModal(false); await Promise.all([utils.students.list.invalidate(), utils.dashboard.byDate.invalidate(), utils.imports.masterlists.invalidate()]); } });
  ...
  const parseImport = async (file: File) => {
    try {
      ...
      const controlIndex = indexOf("CONTROL NO.", "CONTROL NO", "CONTROL NUMBER");
      const idIndex = indexOf("ID NUMBER");
      const nameIndex = indexOf("FIRST NAME");
      const lastNameIndex = indexOf("LAST NAME");
      const middleNameIndex = indexOf("MIDDLE NAME");
      const emailIndex = indexOf("HCDC EMAIL", "EMAIL");
      const programIndex = indexOf("PROGRAM");
      const yearIndex = indexOf("YEAR LEVEL");
      if ([controlIndex, idIndex, nameIndex, lastNameIndex, middleNameIndex, emailIndex, programIndex, yearIndex].some((index) => index < 0)) { toast.error("Missing required columns", { description: "Required: CONTROL NO., ID NUMBER, FIRST NAME, LAST NAME, MIDDLE NAME, HCDC EMAIL, PROGRAM, YEAR LEVEL" }); return; }
      const parsed = rows.slice(1).map((row) => ({ controlNo: String(row[controlIndex] || "").trim(), studentId: String(row[idIndex] || "").trim(), firstName: String(row[nameIndex] || "").trim(), lastName: String(row[lastNameIndex] || "").trim(), middleName: String(row[middleNameIndex] || "").trim(), email: String(row[emailIndex] || "").trim(), program: String(row[programIndex] || "").trim(), yearLevel: Number(row[yearIndex]), barcode: String(row[idIndex] || "").replace(/-/g, "") })).filter((row) => row.studentId || row.firstName || row.lastName || row.controlNo);
      setImportFilename(file.name);
      setImportPreview(parsed);
      const validCount = parsed.filter((row) => isValidStudentId(row.studentId) && row.controlNo && row.firstName && row.lastName && row.middleName && row.email && row.program && row.yearLevel > 0).length;
      toast.success(`${parsed.length} rows ready for review`, { description: `${validCount} match the required 548xxxxx ID format.` });
    ...
    <div className="min-h-screen bg-[#f7f9f7] text-[#1d2b25]">
      ...
      <main className="min-h-screen lg:pl-[248px]">
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8">
          ...
          {view === "imports" && <section className="animate-rise-in space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-sm text-[#718076]">Upload an Excel workbook, review validation results, and import the roster.</p><h2 className="text-2xl font-semibold tracking-[-0.04em] text-[#19362b]">Import students</h2></div><button onClick={() => { const csv = "ID NUMBER,NAME,LAST NAME,YEAR LEVEL\n54800001,Anna,Cruz,5\n"; const blob = new Blob([csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "student-import-template.csv"; anchor.click(); URL.revokeObjectURL(url); }} className="flex items-center gap-2 text-xs font-semibold text-[#174a3a] hover:underline"><ArrowDownToLine className="h-4 w-4" /> Download template</button><button onClick={() => setShowResetModal(true)} className="flex items-center gap-2 text-xs font-semibold text-[#a35d4e] hover:underline"><Trash2 className="h-4 w-4" /> Clear current roster</button></div><label className="flex min-h-[220px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-[#b8d1bf] bg-[#f2f8f3] p-8 text-center transition hover:border-[#6da48a] hover:bg-[#edf5ef]"><Upload className="mb-3 h-8 w-8 text-[#4f8a64]" /><p className="text-sm font-semibold text-[#355447]">Choose an Excel file to upload</p><p className="mt-1 text-xs text-[#819087]">Supports .xlsx, .xls, and .csv · Required columns: CONTROL NO., ID NUMBER, FIRST NAME, LAST NAME, MIDDLE NAME, HCDC EMAIL, PROGRAM, YEAR LEVEL · only 548xxxxx IDs are accepted</p><input type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void parseImport(file); }} /></label>{importPreview.length > 0 && <div className="overflow-hidden rounded-2xl border border-[#dbe5dd] bg-white shadow-[0_10px_30px_rgba(28,66,47,0.05)]"><div className="flex flex-col justify-between gap-3 border-b border-[#edf1ed] px-5 py-4 sm:flex-row sm:items-center"><div><h3 className="text-sm font-semibold text-[#19362b]">Review import · {importFilename}</h3><p className="mt-1 text-xs text-[#819087]">{importPreview.length} student rows found. Invalid rows are kept for review.</p></div><button onClick={() => importMutation.mutate({ filename: importFilename, rows: importPreview })} disabled={importMutation.isPending} className="flex items-center justify-center gap-2 rounded-xl bg-[#174a3a] px-4 py-2.5 text-xs font-semibold text-white disabled:cursor-wait disabled:opacity-50">{importMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} {importMutation.isPending ? "Importing…" : "Complete import"}</button></div><div className="grid gap-3 border-b border-[#edf1ed] p-5 sm:grid-cols-4"><div className="rounded-xl bg-[#f7faf7] p-3"><p className="text-[10px] uppercase tracking-[0.12em] text-[#819087]">Rows found</p><strong className="mt-1 block text-xl text-[#19362b]">{importPreview.length}</strong></div><div className="rounded-xl bg-[#f2f8f3] p-3"><p className="text-[10px] uppercase tracking-[0.12em] text-[#819087]">Valid-looking</p><strong className="mt-1 block text-xl text-[#47735b]">{importPreview.filter((row) => isValidStudentId(row.studentId) && row.controlNo && row.firstName && row.lastName && row.middleName && row.email && row.program && row.yearLevel > 0).length}</strong></div><div className="rounded-xl bg-[#fff7ed] p-3"><p className="text-[10px] uppercase tracking-[0.12em] text-[#819087]">Needs review</p><strong className="mt-1 block text-xl text-[#a06d3f]">{importPreview.filter((row) => !isValidStudentId(row.studentId) || !row.controlNo || !row.firstName || !row.lastName || !row.middleName || !row.email || !row.program || !row.yearLevel).length}</strong></div><div className="rounded-xl bg-[#f4f0ec] p-3"><p className="text-[10px] uppercase tracking-[0.12em] text-[#819087]">Duplicates</p><strong className="mt-1 block text-xl text-[#765a43]">—</strong></div></div><div className="max-h-72 overflow-auto"><table className="w-full min-w-[980px] table-fixed text-left"><thead className="sticky top-0 bg-[#f8faf8] text-[10px] uppercase tracking-[0.14em] text-[#819087]"><tr><th className="w-[110px] px-5 py-3 font-semibold">Control No.</th><th className="px-5 py-3 font-semibold">ID number</th><th className="px-5 py-3 font-semibold">Name</th><th className="px-5 py-3 font-semibold">Program</th><th className="px-5 py-3 font-semibold">Year</th><th className="px-5 py-3 text-right font-semibold">Validation</th></tr></thead><tbody className="divide-y divide-[#edf1ed]">{importPreview.map((row, index) => { const valid = isValidStudentId(row.studentId) && row.controlNo && row.firstName && row.lastName && row.middleName && row.email && row.program && row.yearLevel > 0; return <tr key={`${row.studentId}-${index}`}><td className="w-[110px] px-5 py-3 text-xs text-[#587063]">{row.controlNo || "—"}</td><td className="px-5 py-3 text-xs text-[#587063]">{row.studentId || "—"}</td><td className="px-5 py-3 text-xs font-medium text-[#355447]">{row.firstName} {row.middleName} {row.lastName}</td><td className="max-w-[240px] truncate px-5 py-3 text-xs text-[#587063]">{row.program || "—"}</td><td className="px-5 py-3 text-xs text-[#587063]">{row.yearLevel || "—"}</td><td className="px-5 py-3 text-right">{valid ? <span className="text-xs font-medium text-[#4f8a64]">Ready</span> : <span className="text-xs font-medium text-[#b06e42]">Needs review</span>}</td></tr>; })}</tbody></table></div></div>}<div className="rounded-2xl border border-[#dbe5dd] bg-white shadow-[0_10px_30px_rgba(28,66,47,0.05)]"><div className="border-b border-[#edf1ed] px-5 py-4"><h3 className="text-sm font-semibold text-[#19362b]">Recent import history</h3><p className="mt-1 text-xs text-[#819087]">Validation summaries from the latest uploads.</p></div><div className="divide-y divide-[#edf1ed]">{imports.length ? imports.map((batch) => <div key={batch.id} className="flex flex-col justify-between gap-2 px-5 py-4 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><div className="rounded-xl bg-[#f2e8da] p-2 text-[#825938]"><FileSpreadsheet className="h-4 w-4" /></div><div><p className="text-sm font-semibold text-[#355447]">{batch.filename}</p><p className="mt-1 text-[11px] text-[#819087]">{batch.studentsFound} found · {batch.successfullyImported} imported · {batch.duplicates} duplicates · {batch.invalidRecords} invalid</p></div></div><span className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] ${batch.status === "completed" ? "bg-[#edf4ee] text-[#47735b]" : "bg-[#fff3e4] text-[#a06d3f]"}`}>{batch.status}</span></div>) : <div className="px-5 py-10 text-center text-sm text-[#819087]">No imports yet. Upload a workbook to start your roster.</div>}</div></div><div className="overflow-hidden rounded-2xl border border-[#dbe5dd] bg-white shadow-[0_10px_30px_rgba(28,66,47,0.05)]"><div className="border-b border-[#edf1ed] px-5 py-4"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-[#19362b]">Masterlist history</h3><p className="mt-1 text-xs text-[#819087]">Previous uploads stay here even after the current roster is cleared.</p></div><span className="rounded-full bg-[#edf4ee] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[#47735b]">{masterlists.length} uploads</span></div></div><div className="divide-y divide-[#edf1ed]">{masterlists.length ? masterlists.map((masterlist) => <div key={masterlist.id} className="flex flex-col justify-between gap-2 px-5 py-4 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><div className="rounded-xl bg-[#edf4ee] p-2 text-[#47735b]"><FileSpreadsheet className="h-4 w-4" /></div><div><p className="text-sm font-semibold text-[#355447]">{masterlist.filename}</p><p className="mt-1 text-[11px] text-[#819087]">{masterlist.totalRows} rows · {masterlist.importedRows} imported · {masterlist.rejectedRows} rejected · {masterlist.duplicateRows} duplicates</p></div></div><span className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.1em] ${masterlist.status === "active" ? "bg-[#edf4ee] text-[#47735b]" : masterlist.status === "rejected" ? "bg-[#fff3e4] text-[#a06d3f]" : "bg-[#f1f1ef] text-[#7b827d]"}`}>{masterlist.status}</span></div>) : <div className="px-5 py-10 text-center text-sm text-[#819087]">No masterlists uploaded yet.</div>}</div></div></section>}
        </div>
